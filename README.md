...
**Toda a app é escura e fotográfica.** Não há modo claro nem escolha de tema — a lagoa ao anoitecer é o único material, em todos os ecrãs, sempre. Cada ecrã abre com a fotografia a ocupar o ecrã inteiro e o título assente no fundo dela, como a Hoje: na Hoje a imagem é fixa porque o ecrã não corre; nos outros separadores corre com o conteúdo, mas abre sempre a cheio. O cabeçalho e a barra de navegação são véus, não superfícies — a fotografia passa por baixo deles. Na área da organização a mesma capa é mais curta (`.capa--curta`), para não engolir o ecrã antes de se chegar aos campos. A navegação principal usa um sinal próprio — cinco punções desenhados do vocabulário do passeio (sol nascente, gancho de rota, retícula com curvas de nível, provas empilhadas, filetes em degradé), gravados em três pesos de traço e destacados a latão; é a única navegação da app que não usa o conjunto geral de ícones. A exceção deliberada é o SOS, que continua o único ecrã inteiramente vermelho — sem fotografia, para não distrair de uma emergência.
